...
def listener_process(conversation_started, conversation_ended,  user_speaking, bot_responding, stop_audio):
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    handler = logging.StreamHandler()
    handler.setFormatter(logging.Formatter(f'%(asctime)s -listener - %(levelname)s - %(message)s'))
    ...
    while not conversation_ended.is_set():
        ...
        record_audio(conversation_started, conversation_ended, user_speaking, bot_responding, stop_audio)
        user_speaking.clear()
        logging.info("[Listener] User stopped speaking")

def record_audio(conversation_started, conversation_ended, user_speaking, bot_responding, stop_audio):
    ...
    conversation_duration = 1.5  # Duration of conversation in seconds to consider as a pause
    ...
    while True:
        ...
        if not conversation_started.is_set():
            ...
            if silent_chunks >= max_silent_chunks:
                logging.info("[Listener] Maximum silence duration reached, stopping recording.")
            ...
            if started_talking and silent_chunks > conversation_chunks and silent_chunks < max_silent_chunks:
                logging.info("[Listener] Conversation pause detected.")
                if user_speaking.is_set():
                    logging.info("[Listener] Saving chunk of recording.")
                    file_count += 1
                    save_audio_chunk(frames, f"user_input.wav", audio, audio_format, channels, rate)
                    user_speaking.clear()
    ...
    if all_frames:
        file_count += 1
        save_audio_chunk(all_frames, f"user_conversation.wav", audio, audio_format, channels, rate)
